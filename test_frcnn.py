...
class TestFRCNN:
    def __init__(self, config_output_filename, num_rois):
        ...
        self.class_mapping = C.class_mapping
        ...
        classifier = nn.classifier(feature_map_input, roi_input, C.num_rois, nb_classes=len(self.class_mapping), trainable=True)
        ...
        self.model_rpn.load_weights(C.model_path, by_name=True)
        self.model_classifier.load_weights(C.model_path, by_name=True)
...
def test_folder(img_path, model):
    for idx, img_name in enumerate(sorted(os.listdir(img_path))):
        ...
        for box in pred_boxes:
            cv2.rectangle(img,(box.x1, box.y1), (box.x2, box.y2), (int(model.class_to_color[box.class_name][0]), int(model.class_to_color[box.class_name][1]), int(model.class_to_color[box.class_name][2])),2)

            textLabel = '{}: {}'.format(box.class_name,int(100*box.prob))
            all_dets.append((box.class_name,100*box.prob))

            (retval,baseLine) = cv2.getTextSize(textLabel,cv2.FONT_HERSHEY_COMPLEX,1,1)
            textOrg = (box.x1, box.y1-0)
        ...
        print(all_dets)
        # cv2.imshow('img', img)
        cv2.imwrite('./result_imgs/{}.png'.format(idx),img)
        cv2.waitKey(0)
